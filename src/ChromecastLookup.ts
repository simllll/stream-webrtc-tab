  ...
  constructor(deviceIp?: string) {
    ...
    this.mdns.on("response", (response) => {
      const chromeDevice = response.answers.find(
        (service) =>
          service.type === "PTR" && service.name === "_googlecast._tcp.local"
      );
      if (chromeDevice) {
        const srvEntry = response.additionals.find(
          (service) => service.type === "SRV"
        );
        const txtEntry = response.additionals.find(
          (service) => service.type === "TXT"
        );
        const aEntry = response.additionals.find(
          (service) => service.type === "A"
        );
